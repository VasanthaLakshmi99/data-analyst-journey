...
ORDER BY years_experience DESC
LIMIT 5;
--->The Hospital Director wants to know how many patients are registered in the hospital.
SELECT COUNT(*) AS total_patients
FROM patients;
--->The HR Manager wants to know how many doctors are employed.
SELECT COUNT(*) AS total_doctors
FROM doctors;
--->The Finance Director wants to know the total revenue generated by the hospital.
SELECT SUM(amount) AS total_revenue
FROM billing;
--->The Chief Medical Officer wants to know the average treatment cost.
SELECT AVG(cost) AS average_treatment_cost
FROM treatments;
--->Finance wants to know the highest bill amount.
SELECT MAX(amount) AS highest_bill
FROM billing;
--->The Women's Health Department wants to know how many female patients are registered.
SELECT COUNT(*) AS total_female_patients
FROM patients
WHERE gender = 'F';
--->The HR Department wants to know how many Cardiologists work in the hospital.
SELECT COUNT(*) AS total_cardiologists
FROM doctors
WHERE specialization = 'Cardiologist';
--->Finance wants the total amount of pending bills.
SELECT SUM(amount) AS total_pending_amount
FROM billing
WHERE payment_status = 'Pending';
--->The Hospital Director wants the average years of experience of all doctors.
SELECT AVG(years_experience) AS average_years_experience
FROM doctors;
--->The Medical Department wants the lowest treatment cost.
SELECT MIN(cost) AS lowest_treatment_cost
FROM treatments;
--->The Finance Director wants the average amount of completed payments.
SELECT AVG(amount) AS average_completed_payment
FROM billing
WHERE payment_status = 'Completed';
--->The Operations Manager wants to know how many appointments have been cancelled.
SELECT COUNT(*) AS cancelled_appointments
FROM appointments
WHERE status = 'Cancelled';
--->Finance wants the highest pending bill amount.
SELECT MAX(cost) AS lowest_treatment_cost
FROM treatments;
--->The Medical Director wants the total treatment cost for treatments costing more than ₹5,000.
SELECT SUM(cost) AS total_treatment_cost
FROM treatments
WHERE cost > 5000;
--->The CEO wants to know the highest years of experience among Cardiologists.
SELECT MAX(years_experience) AS highest_experience_among_cardiologists
FROM doctors
WHERE specialization = 'Cardiologist';
--->The CEO wants a KPI showing:Total Patients,Total Doctors.
SELECT COUNT(*) AS total_patients
FROM patients;
SELECT COUNT(*) AS total_doctors
FROM doctors;
--->Finance wants a dashboard KPI showing:Total Bills,Total Revenue.
SELECT COUNT(*) AS total_bills
FROM billing;
SELECT SUM(amount) AS total_revenue
FROM billing;
--->The Medical Director wants the average treatment cost for treatments above ₹10,000.
SELECT AVG(cost) AS average_treatment_cost_above_10000
FROM treatments
WHERE cost > 10000;
--->The HR Department wants the minimum years of experience among all doctors.
SELECT MIN(years_experience) AS minimum_years_experience
FROM doctors;
--->Create a Finance KPI report that returns one row containing:Total Bills,Total Revenue,Average Bill Amount,Highest Bill,Lowest Bill.
SELECT COUNT(*) AS total_bills,
